...
public interface TekstikappaleviiteRepository extends JpaWithVersioningRepository<TekstiKappaleViite, Long>, TekstikappaleviiteRepositoryCustom {
    ...
    @Query(
            nativeQuery = true,
            value ="""
                    WITH RECURSIVE hierarchy(id, vanhempi_id, original_viite_id) AS ( 
                    ...
                    WHERE ops.id = :opsId
                    AND ops.tila <> 'POISTETTU'
                    """)
    List<TekstiKappaleViite> findAllByOpetussuunnitelmaIdAndTekstikappaleTunniste(Long opsId, String tunniste);

    default TekstiKappaleViite findByOpetussuunnitelmaIdAndTekstikappaleTunniste(Long opsId, String tunniste) {
      return findAllByOpetussuunnitelmaIdAndTekstikappaleTunniste(opsId, tunniste).stream().findFirst().orElse(null);
    }
}
